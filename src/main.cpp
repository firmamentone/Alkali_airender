...
//if ( access( "/dev/video0", F_OK ) != 0 )return -1;
//if ( access( "/dev/video2", F_OK ) != 0 )return -1;



//===============  aubox no.11 demo
/*
system("sudo ifconfig eth0 192.168.2.244");
system("sudo xmutil unloadapp");
system("sudo xmutil loadapp myfw8");
system("sudo sleep 2");

        sprintf(pip, " \
rtspsrc location=\"rtsp://root:admin9369@192.168.2.63/live1s1.sdp\" latency=1 ! queue max-size-buffers=1 ! rtph264depay ! queue max-size-buffers=1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec \
! video/x-raw, format=NV12 \
! queue max-size-buffers=1 ! tee name=t16 t16.src_0 ! queue max-size-buffers=1 leaky=2 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/B4096/preprocess.json ! queue max-size-buffers=2 leaky=2 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096/aiinference.json \
! ima16.sink_master ivas_xmetaaffixer name=ima16 ima16.src_master \
! fakesink t16.src_1 ! queue max-size-buffers=2 leaky=2 \
! ima16.sink_slave_0 ima16.src_slave_0 ! queue max-size-buffers=2 leaky=2 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096/drawresult.json ! queue max-size-buffers=2 leaky=2 \
...
system("sudo ifconfig eth0 192.168.2.244");
system("sudo xmutil unloadapp");
system("sudo xmutil loadapp myfw4");
system("sudo modetest -M xlnx -D 80000000.v_mix -s 52@40:3840x2160@NV16 &");
system("sudo sleep 2");
...
! video/x-raw, format=NV12 \
! queue max-size-buffers=1 ! tee name=t16 t16.src_0 ! queue max-size-buffers=1 leaky=2 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/B4096_vmix/preprocess.json ! queue max-size-buffers=2 leaky=2 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/aiinference_refine.json \
! ima16.sink_master ivas_xmetaaffixer name=ima16 ima16.src_master \
! fakesink t16.src_1 ! queue max-size-buffers=2 leaky=2 \
! ima16.sink_slave_0 ima16.src_slave_0 ! queue max-size-buffers=2 leaky=2 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/drawresult.json ! queue max-size-buffers=2 leaky=2 \
! kmssink plane-id=36 bus-id=b0000000.v_mix render-rectangle=\"<0,1080,1920,1080>\" show-preroll-frame=false sync=false \
rtspsrc location=\"rtsp://root:admin9369@192.168.2.64/live1s1.sdp\" latency=1 ! queue max-size-buffers=1 ! rtph264depay ! queue max-size-buffers=1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec \
! video/x-raw, format=NV12 \
! queue max-size-buffers=1 ! tee name=t2 t2.src_0 ! queue max-size-buffers=1 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/B4096_vmix/preprocess.json ! queue max-size-buffers=1 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/other_jsons/aiinference.json \
! ima2.sink_master ivas_xmetaaffixer name=ima2 ima2.src_master \
! fakesink t2.src_1 ! queue max-size-buffers=1 \
! ima2.sink_slave_0 ima2.src_slave_0 ! queue max-size-buffers=1 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/drawresult.json ! queue max-size-buffers=1 \
! kmssink plane-id=34 bus-id=b0000000.v_mix render-rectangle=\"<0,0,1920,1080>\" show-preroll-frame=false sync=false \
rtspsrc location=\"rtsp://root:admin9369@192.168.2.65/live1s1.sdp\" latency=1 ! queue max-size-buffers=1 ! rtph264depay ! queue max-size-buffers=1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec \
! video/x-raw, format=NV12 \
! queue max-size-buffers=2 leaky=2 ! tee name=t60 t60.src_0 ! queue max-size-buffers=1 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/B4096_vmix/preprocess.json ! queue max-size-buffers=1 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/aiinference_refine.json \
! ima60.sink_master ivas_xmetaaffixer name=ima60 ima60.src_master \
! fakesink t60.src_1 ! queue max-size-buffers=1 leaky=2 \
! ima60.sink_slave_0 ima60.src_slave_0 ! queue max-size-buffers=1 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/drawresult.json ! queue max-size-buffers=1 \
! kmssink plane-id=35 bus-id=b0000000.v_mix render-rectangle=\"<1920,0,1920,1080>\" show-preroll-frame=false sync=false \
rtspsrc location=\"rtsp://root:admin9369@192.168.2.13/live1s1.sdp\" latency=1 ! queue max-size-buffers=1 ! rtph264depay ! queue max-size-buffers=1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec \
! video/x-raw, format=NV12 \
! queue max-size-buffers=1 leaky=2 ! tee name=t75 t75.src_0 ! queue max-size-buffers=1 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/B4096_vmix/preprocess.json ! queue max-size-buffers=1 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/aiinference_yolov2.json \
! ima75.sink_master ivas_xmetaaffixer name=ima75 ima75.src_master \
! fakesink t75.src_1 ! queue max-size-buffers=1 leaky=2 \
! ima75.sink_slave_0 ima75.src_slave_0 ! queue max-size-buffers=1 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/drawresult.json ! queue max-size-buffers=1 \
! kmssink plane-id=37 bus-id=b0000000.v_mix render-rectangle=\"<1920,1080,1920,1080>\" show-preroll-frame=false sync=false" 
);
...
system("sudo sleep 2");
        ...
        sprintf(pip, " \
rtspsrc location=\"rtsp://root:admin9369@192.168.2.62/live1s1.sdp\" latency=1 ! queue max-size-buffers=1 ! rtph264depay ! queue max-size-buffers=1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec \
! video/x-raw, format=NV12 \
! queue max-size-buffers=1 ! tee name=t16 t16.src_0 ! queue max-size-buffers=1 leaky=2 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/B4096/preprocess.json ! queue max-size-buffers=2 leaky=2 \
...
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/drawresult.json ! queue max-size-buffers=2 leaky=2 \
! kmssink plane-id=36 bus-id=b0000000.v_mix render-rectangle=\"<0,1080,1920,1080>\" show-preroll-frame=false sync=false \
rtspsrc location=\"rtsp://root:admin9369@192.168.2.99/live1s1.sdp\" latency=1 ! queue max-size-buffers=1 ! rtph264depay ! queue max-size-buffers=1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec \
! video/x-raw, format=NV12 \
! queue max-size-buffers=1 ! tee name=t2 t2.src_0 ! queue max-size-buffers=1 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/B4096_vmix/preprocess.json ! queue max-size-buffers=1 \
...
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/drawresult.json ! queue max-size-buffers=1 \
! kmssink plane-id=34 bus-id=b0000000.v_mix render-rectangle=\"<0,0,1920,1080>\" show-preroll-frame=false sync=false \
rtspsrc location=\"rtsp://root:admin9369@192.168.2.76/live1s1.sdp\" latency=1 ! queue max-size-buffers=1 ! rtph264depay ! queue max-size-buffers=1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec \
! video/x-raw, format=NV12 \
! queue max-size-buffers=2 leaky=2 ! tee name=t60 t60.src_0 ! queue max-size-buffers=1 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/B4096_vmix/preprocess.json ! queue max-size-buffers=1 \
...
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/B4096_vmix/drawresult.json ! queue max-size-buffers=1 \
! kmssink plane-id=35 bus-id=b0000000.v_mix render-rectangle=\"<1920,0,1920,1080>\" show-preroll-frame=false sync=false \
rtspsrc location=\"rtsp://root:admin9369@192.168.2.124/live1s1.sdp\" latency=1 ! queue max-size-buffers=1 ! rtph264depay ! queue max-size-buffers=1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec \
! video/x-raw, format=NV12 \
! queue max-size-buffers=1 leaky=2 ! tee name=t75 t75.src_0 ! queue max-size-buffers=1 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/B4096_vmix/preprocess.json ! queue max-size-buffers=1 \
...
/*
        sprintf(pip, " \
rtspsrc location=\"rtsp://admin:ly28595615@192.168.7.26:554/main\" \
! queue ! rtph264depay ! queue max-size-buffers=2 leaky=2 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec low-latency=true \
! queue max-size-buffers=1 ! tee name=t2 t2.src_0 ! queue max-size-buffers=1 \
! ivas_xmultisrc kconfig=/opt/xilinx/share/ivas/smartcam/refinedet/preprocess.json ! queue \
! ivas_xfilter kernels-config=/opt/xilinx/share/ivas/smartcam/refinedet/aiinference.json \
! ima2.sink_master ivas_xmetaaffixer name=ima2 ima2.src_master \
! fakesink t2.src_1 ! queue max-size-buffers=1 leaky=2 \
! ima2.sink_slave_0 ima2.src_slave_0 ! queue \
! ivas_xfilter kernels-config=/opt/xilinx/share/ivas/smartcam/refinedet/drawresult.json ! queue \
! kmssink driver-name=xlnx plane-id=39 sync=false fullscreen-overlay=true"
);
*/
...
! video/x-h264,width=1920,height=1080,framerate=10/1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec low-latency=true \
! video/x-raw, format=NV12 \
! queue max-size-buffers=1 ! tee name=t2 t2.src_0 ! queue max-size-buffers=1 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/demo_B4096_vmix/preprocess.json ! queue \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/demo_B4096_vmix/other_jsons/aiinference.json \
! ima2.sink_master ivas_xmetaaffixer name=ima2 ima2.src_master \
! fakesink t2.src_1 ! queue max-size-buffers=1 \
! ima2.sink_slave_0 ima2.src_slave_0 ! queue max-size-buffers=1 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/demo_B4096_vmix/drawresult.json ! queue max-size-buffers=10 \
! kmssink plane-id=34 bus-id=b0000000.v_mix render-rectangle=\"<0,0,1920,1080>\" show-preroll-frame=false sync=false \
v4l2src device=/dev/video2 \
! video/x-h264,width=1920,height=1080,framerate=10/1 \
! h264parse ! queue max-size-buffers=1 ! omxh264dec low-latency=true \
! video/x-raw, format=NV12 \
! queue max-size-buffers=1 ! tee name=t16 t16.src_0 ! queue max-size-buffers=1 \
! ivas_xmultisrc kconfig=/etc/rc5.d/my_tmp/demo_B4096_vmix/preprocess.json ! queue \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/demo_B4096_vmix/aiinference_yolov2.json \
! ima16.sink_master ivas_xmetaaffixer name=ima16 ima16.src_master \
! fakesink t16.src_1 ! queue max-size-buffers=1 \
! ima16.sink_slave_0 ima16.src_slave_0 ! queue max-size-buffers=1 \
! ivas_xfilter kernels-config=/etc/rc5.d/my_tmp/demo_B4096_vmix/drawresult.json ! queue max-size-buffers=10 \
! kmssink plane-id=36 bus-id=b0000000.v_mix render-rectangle=\"<1920,1080,1920,1080>\" show-preroll-frame=false sync=false");
*/
//! kmssink driver-name=xlnx plane-id=39 sync=false fullscreen-overlay=true \
